...
abstract class sfAction extends sfComponent
{
  ...
  {
	$configuracion = sfContext::getInstance()->getUser()->getAttribute("configuracion");
	
	if(!$configuracion){
		$configuracion = Doctrine_Core::getTable('Configuracion')->find(array('id'=>1));
		sfContext::getInstance()->getUser()->setAttribute("configuracion", $configuracion);
	}
	
	$valores = sfContext::getInstance()->getUser()->getAttribute("valores");
	
	if(!$valores ){
		$valores = Doctrine_Core::getTable("Valores")->find(array("id"=>1));
		sfContext::getInstance()->getUser()->setAttribute("valores", $valores);
	}
  }
